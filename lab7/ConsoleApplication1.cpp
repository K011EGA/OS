...
void MonitorStack::Push(long& nElement) {
    arr[count] = nElement;
    addCount();
}

long MonitorStack::Pop() {
    minusCount();
    return arr[count];
}
...
public:
    ...
    tagParam(long* products, int size, HANDLE hAccess, HANDLE hFull, HANDLE hEmpty) : MonitorStack(size)
    {
        count = 0;
        iterator = -1;
...
int main()
{
    ...
    products = new long[countProducer];
    for (int i = 0; i < countProducer; i++) {
        products[i] = rand() * 100000000 % 2147483648;
    }
